...
kubelet cannot be not be secured.
...
- Find and edit the `kubelet-config-x.yy` ConfigMap under `kube-systedm` to include
...
approved by the default signer in kube-controller-manager. This will require action from
...
Third party custom controllers such as
[kubelet-rubber-stamp](https://github.com/kontena/kubelet-rubber-stamp) can also be used.
